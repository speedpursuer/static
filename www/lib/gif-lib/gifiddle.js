/*
function getFile (fileName, gifiddle) {
    var rawFile = new XMLHttpRequest();
    rawFile.open("GET", fileName, true)
    rawFile.responseType = 'arraybuffer';
    rawFile.onload = function(e) {      
        gifiddle.loadBuffer(this.response);
    };            
    rawFile.send();
}
function getFile (fileName, gifiddle) {
...
function Gifiddle() {
    ...
    return {
        ...
        setup: function(gif) {
            ...
            player = new GifPlayer(domViewport[0], 1);
            player.setup(gif);
...
function GifiddleControls(gifiddle) {
    ...
    */
    ...
    function updateIcon_old(playing) {
        if (playing) {
            domIconPlay.hide();
            domIconPause.show();
        } else {
    ...
    gifiddle.events.on('initPlayer', function(gifPlayer) {
        ...
        domButtons.on('click', function(event) {
    		...
    		if (!cmd)
			{
				var s = event.target.classList[1];
									
				if (s.indexOf("fa-step-backward") > -1){				
					cmd = "setPrevious";
				} else if (s.indexOf("fa-step-forward") > -1) {
					cmd = "setNext";				
				} else if (s.indexOf("fa-play") > -1 || s.indexOf("fa-pause") > -1) {
					cmd = "toggle";					
				}
			}*/
