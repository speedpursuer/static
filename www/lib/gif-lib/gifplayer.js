...
function GifPlayer(canvas) {
    ...
    function resizeScreen() {
        ...
        var playingBefore = playing;
        instance.pause();
        
        var gameArea = document.getElementById('gameArea');
        var widthToHeight = imageWidth / (imageHeight + 50);
        ...
        frameIndexPrev++;
        instance.setFrameIndex(frameIndexCurr);
    ...
    function resizeScreen_old() {
        ...
        var gameArea = document.getElementById('gameArea');
        var widthToHeight = imageWidth / imageHeight;
        var newWidth = window.innerWidth;
        var newHeight = window.innerHeight;
        var newWidthToHeight = newWidth / newHeight;
    ...
    var instance = {
        events: new Events(),
        setup: function(gif_gif) {
            gif = gif_gif;
            /*
            gif.events.on('load', function() {                
                imageWidth = gif.hdr.width;
                imageHeight = gif.hdr.height;                                
                ready = true;
                resizeScreen();
                this.setFirst();
        ...
        start: function() {
            ...
            resizeScreen();
            this.setFirst();
            this.events.emit('readyToPlay', gif); 
            gif.events.on('finish', function() {                
                this.events.emit('showCtrl');
                pace = 1;
            }.bind(this));  
        },
        start_old: function() {
        ...
        load: function(arrayBuffer) {
            ...
            gif = new GifFile();
            gif.byteLength = arrayBuffer.byteLength;
            gif.events.on('load', function() {
...
GifFile.prototype = {
    ...
    load: function(buffer) {
        ...
        if (useWorker) {        	
            this.worker = new Worker('lib/gif-lib/gifworker.js');
...
GifFrame.prototype = {
    blit: function(ctx) {
        ...
        //ctx.drawImage(this.canvas, this.left, this.top, this.width, this.height);
        ctx.drawImage(this.canvas, this.left, this.top, ctx.canvas.width, ctx.canvas.height);                
    },
    repair: function(ctx) {
        if (!this.gce) {
            return;
